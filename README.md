...
- **Service coverage** - the same city overview reports road, power, fire, police, health, and education coverage across developed zones, with a live health badge and emphasized gap rows so infrastructure problems are visible without opening individual tiles.
- **Live minimap** - a compact terrain-backed overview redraws from typed city data when buildings are placed or removed; zones appear as colored parcels, roads as connected streets, power lines as a thinner node network, and utilities/services as facility markers. A live focus ring tracks camera panning, hovering previews a tile's category and coordinates, and clicking any map location recenters the camera there.
...
- **Actionable city overview** - zone-capacity rows and icon-led service cards show live utilization, coverage, covered-zone counts, and urgent gaps; unlocked entries jump directly to the matching zoning or infrastructure tool.
...
| Collapse HUD panels                         | Click the Milestones, Zone capacity, or Controls header; each preference is remembered locally                                                  |
