...
- **InfoPanel** - receives an `InspectorUiState`, built by `game/inspector.ts` from the focused tile. It renders service status, development, costs, power capacity, and citizen/worker occupancy as React elements. Its close action clears both `Game.focusedObject` and the render-layer selection highlight, so hidden inspector state cannot remain active in the world. No simulation object generates markup and no model data crosses through `innerHTML`.
...
- **ZoneCapacityPanel** - shows developed residential occupancy and developed commercial/industrial staffing as three compact utilization bars, followed by road, power, fire, police, health, and education coverage across developed zones. `game/zoneCapacity.ts` and `game/cityServices.ts` derive these values from real resident, job, access, and civic-coverage attributes; empty rows explicitly say there are no active zones rather than presenting synthetic demand.
...
- **CityMap** - draws a compact top-down map on a dedicated 2D HUD canvas from `CityMapUiState`; the UI never imports Three.js or reads `City` directly. `game/cityMap.ts` projects every building into a small display category (zones, road, power, civic service), and `Game` batches placement/removal bursts into one microtask redraw so loading a save does not rescan the entire city per tile. The map shares a lower-left stack with the controls disclosure, hides while the full controls reference is open, and is omitted on narrow or short displays.
