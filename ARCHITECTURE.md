...
- **ActivityLog** - the same store operation that shows a toast also prepends it to a six-entry, session-only history with the current simulation day. The city-management menu renders that bounded history after the toast disappears; it deliberately does not enter the simulation save format.
