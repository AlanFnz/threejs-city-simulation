...
- **`Game`** (`subscribeToCityEvents()`) - UI-only reactions: `citizenMovedIn`/`citizenMovedOut`/`moneyChanged`/`economyUpdated` update the top bar and budget breakdown; every tile event calls `refreshInfoOverlayIfFocused(payload)`, which only re-renders the info panel if the changed tile is the one currently selected; `milestoneCompleted` and `randomEventTriggered` refresh the goals panel / surface a typed notification.
...
One save slot in `localStorage` (key `threejs-city-simulation/save`), versioned (`SaveGameV1.version: 1`) for future migrations. `Game` auto-loads on construction (`this.loadGame()`) and autosaves every 30 sim ticks (`AUTOSAVE_INTERVAL_TICKS`), plus explicit Save/Load/New-City actions in the top bar's management menu. The player-defined city name is game metadata owned by `Game`; renaming writes the slot immediately, and old version-1 saves without the optional `cityName` field fall back to `My City`.

- **`serialize(city, milestoneTracker, cityName)`** walks every tile and records `{x, y, buildingType}` plus, for zones, `{style, rotation, developmentState, developmentLevel, developmentMaxLevel}` and, for `ResidentialZone`, each citizen `{id, firstName, surname, age, state, workplace?}`. Also captures the normalized city name, `city.money`, `city.upkeepDiscount`, a snapshot of `ZONE_LEVEL_CAPS`, and `milestoneTracker.getState()` (`{completed, unlockedToolIds}`).
...
- **TopBar** - editable city name, population counter (`#population-counter`), money balance, and per-tick net-income trend. Enter or blur commits a name edit, Escape cancels it, and `Game.renameCity` normalizes and persists the result. Simulation values refresh from `citizenMovedIn`/`citizenMovedOut`/`moneyChanged`/`economyUpdated` events rather than unconditional UI polling. Clicking city funds opens an event-fed budget breakdown of revenue, upkeep, and net flow. Save, load, and new-city actions live in a separate accessible management dropdown; the two popovers are mutually exclusive and close on outside click or Escape (returning focus to their trigger).
