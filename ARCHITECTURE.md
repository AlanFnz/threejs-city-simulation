...
Orthographic camera orbiting a `cameraOrigin` on a sphere (azimuth/elevation/radius-as-zoom). Held WASD input moves the origin camera-relatively and Q/E changes its azimuth from the independent render loop using a clamped frame delta plus `CONFIG.CAMERA.KEYBOARD_PAN_SPEED`/`KEYBOARD_ROTATION_SPEED`; diagonal movement is normalized and scales inversely with zoom so navigation stays useful on larger maps. Right-drag still provides free azimuth/elevation orbit, Ctrl+right-drag pans, wheel zooms, and two-finger touch pans (no pinch-zoom yet). `camera.zoom = cameraRadius` - "radius" is really zoom level. The origin (map center), starting zoom (framed to show the whole map plus margin), and zoom-out limit all derive from `citySize` passed into the constructor, rather than being tuned only for size 16 - `MIN_CAMERA_RADIUS_AT_SIZE_16` in `constants.ts` documents its own scaling factor (`× 16/citySize`) in its name.
...
- **ControlsLegend** - renders the desktop keyboard/mouse camera quick reference inside a native `<details>` disclosure that defaults closed, remains one click away, and remembers its preference independently from the city save. It is hidden on touch or short displays so it never competes with city controls.
