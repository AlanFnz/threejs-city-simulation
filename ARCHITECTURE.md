...
Orthographic camera orbiting a `cameraOrigin` on a sphere (azimuth/elevation/radius-as-zoom). Held WASD input moves the origin camera-relatively from the independent render loop using a clamped frame delta and `CONFIG.CAMERA.KEYBOARD_PAN_SPEED`; diagonal input is normalized and scales inversely with zoom so navigation stays useful on larger maps. Right-drag orbits, Ctrl+right-drag pans, wheel zooms, and two-finger touch pans (no pinch-zoom yet). `camera.zoom = cameraRadius` - "radius" is really zoom level. The origin (map center), starting zoom (framed to show the whole map plus margin), and zoom-out limit all derive from `citySize` passed into the constructor, rather than being tuned only for size 16 - `MIN_CAMERA_RADIUS_AT_SIZE_16` in `constants.ts` documents its own scaling factor (`× 16/citySize`) in its name.
