...
- **`Game`** (`subscribeToCityEvents()`) - UI-only reactions: `citizenMovedIn`/`citizenMovedOut`/`citizenEmployed`/`citizenUnemployed` refresh the population counter and derived census, while `moneyChanged`/`economyUpdated` update the balance and budget breakdown; every tile event calls `refreshInfoOverlayIfFocused(payload)`, which only re-renders the info panel if the changed tile is the one currently selected; `milestoneCompleted` and `randomEventTriggered` refresh the goals panel / surface a typed notification. Employment events within one simulation stack are batched into a microtask before the census scans residents, so final workplace links are visible and a mass layoff produces one UI update rather than one per worker.
